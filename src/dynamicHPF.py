...
class DynamicHPF(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=3, stride=1, pad_type='reflect', group=2,inv = False):
        ...
        self.pad = get_pad_layer(pad_type)(kernel_size//2) # we use nn.ReflectionPad2d and pad it by 3//2 zeros.Pads the input tensor using the reflection of the input boundary.
        self.stride = stride
        self.kernel_size = kernel_size
        self.group = group
        self.inv = inv

        self.conv = nn.Conv2d(in_channels, group*kernel_size*kernel_size, kernel_size=kernel_size, stride=1, bias=False) #input b,32,H,W then output is b,18,H,W
        self.bn = nn.BatchNorm2d(group*kernel_size*kernel_size) #b,18,H,W
        self.softmax = nn.Softmax(dim=1) 
        nn.init.kaiming_normal_(self.conv.weight, mode='fan_out', nonlinearity='relu')

    def forward(self, x):
        sigma = self.conv(self.pad(x)) #b,18,H,W = #b,2*9,H,W ie 2 groups
        sigma = self.bn(sigma) #b,18,H,W
        ...
        for i in range(self.group):
            ...
            # groups of 9 elements are separated out channelwise along dim=1 and softmax is applied.  After applying softmax along the channel dimension, group  they add up to 1 along channel dimension in each spatial location
            sigma[:,(self.kernel_size**2)*i + (self.kernel_size**2)//2,:,:] -= 1 # add -1 from center value of the the k^2 values in dim=1. This gives the condition for HPF, base paper does LPF ; each group sums to 0 along channel dimension in spatial location
