...
def load_data(
        ...
        data_info_list_path,
        batch_size,
        acc_factors=<acceleration factor> #example ['5x'],
        dataset_types=<anatomy type> #example ['cardiac'],
        mask_types=<mask type> # example['cartesian'],
        train_or_test=<train or test flag> #example 'train',
        random_flip=False,
...
#     )
    ...
    dataset = SliceData(data_dir, #args.train_path
                       acc_factors, 
                       dataset_types,
                       mask_types,
                       train_or_test, 
...
#             image_zf  = data[key_img][:,:,slice] #input_img
#             image_zf = torch.from_numpy(image_zf).unsqueeze(0)
            kspace_zf  = data[key_kspace][:,:,slice] #input_kspace
            kspace_zf = npComplexToTorch(kspace_zf).permute(2,0,1) #input_kspace
            image_zf = torch.fft.ifft2(torch.view_as_complex(kspace_zf.permute(1,2,0)), norm="ortho")
            image_zf = torch.view_as_real(image_zf).permute(2,0,1)
            image = data['volfs'][:,:,slice].astype(np.float64)# converting to double #target
            image = torch.from_numpy(image)#.unsqueeze(0)
            kspace = torch.fft.fft2(image, norm='ortho')
            kspace = torch.view_as_real(kspace).permute(2,0,1)
            image = torch.fft.ifft2(torch.view_as_complex(kspace.permute(1,2,0)), norm="ortho")
            image = torch.view_as_real(image).permute(2,0,1)
            scale_coeff = 1. / np.max(np.abs(image_zf.numpy()))
            ...
            mask = np.load(os.path.join(self.mask_path,dataset_type,mask_type,'mask_{}.npy'.format(acc_factor)).translate(translator)).astype(np.float32)
            mask = np.stack([mask,mask])
            mask = torch.from_numpy(mask).float()
            mask_c = 1. - mask
            
            kspace_c = mask_c * kspace
#             print(f'fname: {fname}')
            args_dict = {
            "image": image.float(),
            "image_zf": image_zf.float(),
            "kspace": kspace.float(),
            "kspace_zf": kspace_zf.float(),
            "mask": mask.float(),
            "mask_c": mask_c.float(),
            "scale_coeff": scale_coeff,
#             "acquisition": acquisition,
            "file_name": str(fname).split('/')[-1],
            "slice_index": slice,
            }
...
#     transformed_image = np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(kspace_raw), norm="ortho"))
    transformed_image = np.fft.ifft2(kspace_raw, norm="ortho")
