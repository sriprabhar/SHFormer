...
#         print(f'base_test_util, len: {len(self.data)}')
        for data_item in self.data:
            ...
            FileName = str(os.path.join(self.output_dir, file_name+f"_{slice_index}.h5"))
            if FileName in df['FileName']:
                break
            df = self.forward_backward(data_item, df)
            self.step += 1
        ...
        result_str = f"PSNR = {mean(df['PSNR'])} +/- {2*stdev(df['PSNR'])}, SSIM = {mean(df['SSIM'])} +/- {2*stdev(df['SSIM'])}, MSE = {mean(df['MSE'])} +/- {2*stdev(df['MSE'])}, NMSE = {mean(df['NMSE'])} +/- {2*stdev(df['NMSE'])}"
