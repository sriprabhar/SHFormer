...
#         self.compute_metrics_for_dataset()
    ...
    def forward_backward(self, data_item, df):
        kspace_c, batch_kwargs = data_item
        file_name = str(batch_kwargs["file_name"][0]).split('.')[0]
        slice_index = str(int(batch_kwargs["slice_index"].numpy()))
#         print(f'ddpm_test_util, file_name: {file_name}, slice_index: {slice_index}')
        samples_path = os.path.join(self.output_dir, file_name, f"slice_{slice_index}")
        if not os.path.exists(os.path.join(self.output_dir, file_name)):
            os.makedirs(os.path.join(self.output_dir, file_name), exist_ok=True)
...
#             os.makedirs(samples_path, exist_ok=True)
        ...
        print(f'Sampling time: {abs(end-start)}')
        image = np.abs(real2complex_np(batch_kwargs["image"][0].numpy()))[np.newaxis,:,:]
        recon = np.abs(real2complex_np(samples[0]))[np.newaxis,:,:]
#         print(compute_psnr(image, recon), compute_ssim(image, recon))
        df['FileName'].append(str(os.path.join(self.output_dir, file_name+f"_{slice_index}.h5")))
        df['PSNR'].append(compute_psnr(image, recon))
        df['SSIM'].append(compute_ssim(image, recon))
        df['MSE'].append(compute_mse(image, recon))
        df['NMSE'].append(compute_nmse(image, recon))
...
#         mask = batch_kwargs["mask"][0, 0, ...]
        ...
        image = batch_kwargs["image"][0].numpy()
#         print(f'ddpm_test_util, samples: {samples.shape}, image: {image.shape}')
        recon = samples[0]
        
#         print(f'ddpm_test_util, samples_path: {samples_path}')
        with h5.File(str(samples_path)+'.h5','w') as file:
            file.create_dataset("recons", data=recon)
            file.create_dataset("gt", data=image)
